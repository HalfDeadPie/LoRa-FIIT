...
void RH_RF95::setModeRx()
{
	...
	spiWrite(RH_RF95_REG_40_DIO_MAPPING1, 0xC0); // Interrupt on RxDone
	_mode = RHModeRx;
    }
}
...
void RH_RF95::setModeTx()
{
	...
	spiWrite(RH_RF95_REG_40_DIO_MAPPING1, 0xF0); // Interrupt on TxDone
	_mode = RHModeTx;
    }
}
...
void RH_RF95::setModemRegisters(const ModemConfig* config)
{
	...
	/*Serial.println("FIIT DEBUG:");
	Serial.println(spiRead(RH_RF95_REG_1D_MODEM_CONFIG1),BIN);
	Serial.println(spiRead(RH_RF95_REG_1E_MODEM_CONFIG2),BIN);
	Serial.println(spiRead(RH_RF95_REG_26_MODEM_CONFIG3),BIN);*/
}
...
void RH_RF95::setLoraFiit(byte bwin, byte crin, byte sfin, bool mobile){
	spiWrite(RH_RF95_REG_33_INVERT_IQ, spiRead(RH_RF95_REG_33_INVERT_IQ)|(1<<6));
	
	byte bw = spiRead(RH_RF95_REG_1D_MODEM_CONFIG1);
	bw &= B00001111;//bw
	bw |= bwin;
	
	bw &= B11110000;//cr and explicit header mode
	bw |= crin;

	
	byte sf = spiRead(RH_RF95_REG_1E_MODEM_CONFIG2);//SF
	sf &= B00001111;
	sf |= sfin;
	
	byte special = spiRead(RH_RF95_REG_26_MODEM_CONFIG3);
	special &= B11110011;
	if (mobile == true) {
		special |= B00001100;
	}	 
	...
	ModemConfig lorafiit;
	lorafiit.reg_1d = bw;
	lorafiit.reg_1e = sf;
	lorafiit.reg_26 = special;
	
	setModemRegisters(&lorafiit);
}
